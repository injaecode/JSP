...
<%
	...
	//답변을 처리하는 컬럼 3개: masterid replynum step
	String sub=null;	//테이블의 제목을 저장하는 변수 :subject
	String cont=null;	//테이블의 글 내용을 저장하는 변수 :content
	int step=0;			//freeboard테이블의 step칼럼 값을 저장
	int mid=0;			//freeboard테이블의 masterid컬럼 값을 저장
	int rnum=0;			//freeboard테이블의 replynum컬럼값 저장
	...
	if(!(rs.next())){
		...
		cont=">"+rs.getString("content");//DB의 내용글을 가져와서 글앞에 ">" 입력
  ...
  %>
  <FORM name='msgrewrite'  method=POST action="freeboard_rsave2.jsp">
